...
import { LucideAngularModule, Users, Plus, ArrowRight, Wallet } from 'lucide-angular';
        ...
        <div *ngIf="(groups$ | async) as groups; else loading">
          ...
          <div *ngIf="groups.length === 0" class="text-center py-12 bg-white rounded-[32px] border border-slate-100 shadow-sm px-6">
            <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4">
              <lucide-icon [name]="UsersIcon" class="w-10 h-10 text-slate-300"></lucide-icon>
            </div>
            <h3 class="text-lg font-bold text-slate-900 mb-2">Aucun groupe</h3>
            <p class="text-slate-500 text-sm mb-8">Créez votre premier groupe pour commencer à partager vos dépenses.</p>
          </div>
          ...
          <div class="grid gap-4">
            <div
              *ngFor="let group of groups"
              ...
              class="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all group"
            >
              ...
              <div class="w-10 h-10 rounded-xl flex items-center justify-center text-slate-300 group-hover:text-indigo-600 group-hover:bg-indigo-50 transition-all">
...
export class SharedExpensesComponent {
  ...
  constructor() {
    this.groups$ = this.authService.user$.pipe(
      switchMap(user => {
        if (user) {
          return this.sharedService.getGroups(user.uid);
        }
        return of([]);
      })
    );
